...
class FoodVendor
{
    ...
    */
    ...
    public static function getById( $id ) {
      ...
      $row = $st->fetch();
      $conn = null;
      if ( $row ) return new FoodVendor( $row );
    }
    ...
    */
    ...
    public static function getList( $numRows=1000000 ) {
      $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
      $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM vendors
    ...
    */
    ...
    public function insert() : int{
      ...
      if ( !is_null( $this->id ) ) trigger_error ( "FoodVendor::insert(): Attempt to insert a User object that already has its ID property set (to $this->id).", E_USER_ERROR );
      // Insert the user
      $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    ...
    */
    ...
    public function update() :bool{
      ...
      if ( is_null( $this->id ) ) trigger_error ( "FoodVendor::update(): Attempt to update a User object that does not have its ID property set.", E_USER_ERROR );
    
      // Update the user
      $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
      $sql = "UPDATE vendors SET first_name=:first_name, last_name=:last_name, email=:email, modified=:modified WHERE id = :id";
      $st = $conn->prepare ( $sql );
      $st->bindValue( ":first_name", $this->firstname, PDO::PARAM_STR );
      $st->bindValue( ":last_name", $this->lastname, PDO::PARAM_STR );
      $st->bindValue( ":email", $this->email, PDO::PARAM_STR );
    ...
    */
    public function updatePassword() {
      ...
      // Update the user
      $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
      $sql = "UPDATE vendors SET password=:password, modified=:modified WHERE email = :email";
      $st = $conn->prepare ( $sql );
      $st->bindValue( ":password", $this->hashPassword($this->password), PDO::PARAM_STR );
      $st->bindValue( ":modified", date("Y/m/j G.i:s", time()), PDO::PARAM_STR );
      $st->bindValue( ":email", $this->email, PDO::PARAM_STR );
    ...
    public function updatePasswordByID($oldPassword) {
      ...
      // Update the user
      $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
      $sql = "UPDATE vendors SET password=:password, modified=:modified WHERE id = :id AND password = :oldPassword";
      $st = $conn->prepare ( $sql );
      $st->bindValue( ":password", $this->hashPassword($this->password), PDO::PARAM_STR );
    ...
    */
    ...
    public function delete() {
      ...
      // Does the user object have an ID?
      if ( is_null( $this->id ) ) trigger_error ( "FoodVendor::delete(): Attempt to delete a User object that does not have its ID property set.", E_USER_ERROR );

      // Delete the user
      $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
      $st = $conn->prepare ( "DELETE FROM vendors WHERE id = :id LIMIT 1" );
      $st->bindValue( ":id", $this->id, PDO::PARAM_INT );
      $st->execute();
      $conn = null;
    }
